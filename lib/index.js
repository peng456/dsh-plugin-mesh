...
function applyUnsafe(ctx, config) {
  ...
  startPeerServer(ctx, { config, discovery, inbound, log, warn, waker })
    .then(async (port) => {
...
function handleInboundTask(ctx, task, inbound, config, log, warn, waker) {
  const rec = inbound.find((t) => t.id === task.id);

  // 路径 A：原会话仍活跃 → 直接注入，保持对话连续
...
function startPeerServer(ctx, { config, discovery, inbound, log, warn, waker }) {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (path === "/mesh/v1/task" && req.method === "POST") {
      ...
      if (!duplicate) {
        try {
          handleInboundTask(ctx, task, inbound, config, log, warn, waker);
        } catch (err) {
